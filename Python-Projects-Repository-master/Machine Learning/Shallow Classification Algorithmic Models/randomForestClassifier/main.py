...
randomForest_Dataframe = pd.read_csv((
...
randomForest_Dataframe.shape

# Depict how our dataset actually looks
randomForest_Dataframe.head()

# # The purpose of the python functions that follow is to iterate over a dataframe for null, NaN, empty values that affect
# print(randomForest_Dataframe.isnull().any())
# # print(randomForest_Dataframe.index[np.isinf(randomForest_Dataframe).any(1)])
# print(randomForest_Dataframe['Label'].isnull().values.any())
# print(randomForest_Dataframe['Label'].isnull().sum())
# new = randomForest_Dataframe['Label'].loc[randomForest_Dataframe['Label'].isnull()]
# print(new)
# randomForest_Dataframe['Label'].loc[randomForest_Dataframe['Label'].isnull()] = 0
# print(randomForest_Dataframe['Label'].isnull().sum())
# new = randomForest_Dataframe['Label'].loc[randomForest_Dataframe['Label'].isnull()]
# print(new)
# randomForest_Dataframe.to_csv((
...
# print(new)
# randomForest_Dataframe = randomForest_Dataframe.fillna(lambda x: x.median())
# print(randomForest_Dataframe.isnull().any())

# Divide the Label column from the rest of the dataframe.
X = randomForest_Dataframe.drop('Label', axis=1)
y = randomForest_Dataframe['Label']
...
rf_classifier = RandomForestClassifier(n_estimators=1000, random_state=42, criterion="entropy",
                                       max_depth=7, max_features=0.4, min_samples_leaf=3)
...
for train, test in skf.split(inputs, targets):
    ...
    # print(y_train_skf)
    # print(y_test_skf)
    rf_classifier.fit(X_train_skf, y_train_skf)  # Naive Bayes model nb.fit algorithm training.
    y_pred_skf = rf_classifier.predict(X_test_skf)  # y_pred_skf variable is used to store in memory the prediction of
    ...
    plot_confusion_matrix(rf_classifier, X_test_skf, y_test_skf)
    plt.show()
