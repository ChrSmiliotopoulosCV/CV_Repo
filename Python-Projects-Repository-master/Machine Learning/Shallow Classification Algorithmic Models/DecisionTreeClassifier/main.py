...
decisionTree_Dataframe = pd.read_csv((
    r"C:\Users\chrsm\Desktop\AWID3 Project Tasks [14-11-21]\Dataset Subsets\all-without-scale02(noNull-index-true)-reduced.csv"), low_memory=False)

# See the rows and columns of the dataset.
decisionTree_Dataframe.shape

# Depict how our dataset actually looks
decisionTree_Dataframe.head()

# # The purpose of the python functions that follow is to iterate over a dataframe for null, NaN, empty values that affect
# print(decisionTree_Dataframe.isnull().any())
# # print(decisionTree_Dataframe.index[np.isinf(decisionTree_Dataframe).any(1)])
# print(decisionTree_Dataframe['Label'].isnull().values.any())
# print(decisionTree_Dataframe['Label'].isnull().sum())
# new = decisionTree_Dataframe['Label'].loc[decisionTree_Dataframe['Label'].isnull()]
# print(new)
# decisionTree_Dataframe['Label'].loc[decisionTree_Dataframe['Label'].isnull()] = 0
# print(decisionTree_Dataframe['Label'].isnull().sum())
# new = decisionTree_Dataframe['Label'].loc[decisionTree_Dataframe['Label'].isnull()]
# print(new)
# decisionTree_Dataframe.to_csv((
...
# print(new)
# decisionTree_Dataframe = decisionTree_Dataframe.fillna(lambda x: x.median())
# print(decisionTree_Dataframe.isnull().any())

# Divide the Label column from the rest of the dataframe.
X = decisionTree_Dataframe.drop('Label', axis=1)
y = decisionTree_Dataframe['Label']
...
for train, test in skf.split(inputs, targets):
    ...
    # print(y_train_skf)
    # print(y_test_skf)
    classifier.fit(X_train_skf, y_train_skf)  # Naive Bayes model nb.fit algorithm training.
    y_pred_skf = classifier.predict(X_test_skf)  # y_pred_skf variable with the prediction of the classification model.
    ...
    plot_confusion_matrix(classifier, X_test_skf, y_test_skf)
    plt.show()
