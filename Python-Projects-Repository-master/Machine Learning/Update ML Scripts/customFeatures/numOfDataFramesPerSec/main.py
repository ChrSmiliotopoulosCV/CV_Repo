# Data Frames per second counting python script
# Importing Libraries
import pandas as pd
import sklearn
...
data['numOfDataFramesPerSec()'] = data['numOfDataFrames'] / data['Time since reference or first frame']

# New numOfProtectedPacketsPerSec Dataframe which gathers all the requested information towards the calculation of the
# numOfDataFramesPerSec().
numOfDataFramesPerSec = pd.DataFrame(data['Frame Type'])
numOfDataFramesPerSec['Time since reference or first frame'] = data['Time since reference or first frame']
numOfDataFramesPerSec['numOfDataFrames'] = data['numOfDataFrames']
numOfDataFramesPerSec['numOfDataFramesPerSec()'] = data['numOfDataFramesPerSec()']