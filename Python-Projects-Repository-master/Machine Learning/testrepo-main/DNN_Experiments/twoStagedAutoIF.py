...
# path = r"/Users/christossmiliotopoulos/Documents/GitHub/testrepo/DNN_Experiments/twoStagedlogFile02.txt"
path = r"C:\Users\chrsm\Desktop\Paper_no4 - Unsupervised ML LMD-2023 Paper\GitHub_Repository\testrepo\DNN_Experiments\twoStagedlogFile06.txt"
...
# model = IsolationForest(n_estimators=100, verbose=99, random_state=42, contamination=0.2)
random_state = np.random.RandomState(42)
model = IsolationForest(n_estimators=100,max_samples='auto',contamination=float(0.15),random_state=random_state)
# model = LocalOutlierFactor(n_neighbors=3, contamination='auto', novelty=True)

print(type(threshold_prediction))
res = np.array(threshold_prediction)
print(type(res))
res = res.reshape(-1, 1)
...
# pdScores['Scores'] = model.score_samples(X_train)
# print(pdScores)

# pdScores['anomaly_classification'] = model.predict(X_train)
# print(pdScores)

# pdScores['res'] = res
# print(pdScores)

# pdScores['y_test'] = y_train
# print(pdScores)

# pdScores.to_csv('anomalyDetection.csv')
...
conf_matrix = confusion_matrix(y_train, y_pred1)
print(conf_matrix)

# print(classification_report(y_test, y_pred, labels=[0, 1, 2], digits=4))
print(classification_report(y_train, y_pred1, digits=4))
...
y_pred = lb.transform(y_pred1)
print("ROC AUC score:", roc_auc_score(y_train, y_pred, average="macro"))

RocCurveDisplay.from_predictions(y_train, y_pred1)
plt.show()
